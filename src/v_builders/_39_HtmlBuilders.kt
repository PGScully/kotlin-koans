...
fun renderProductTable(): String {
    return html {
        table {
            ...
            products.forEach {
                tr {
                    td(color = getCellColor(row = products.indexOf(it), column = 1)) {
                        text(it.description)
                    }
                    td(color = getCellColor(row = products.indexOf(it), column = 2)) {
                        text(it.price)
                    }
                    td(color = getCellColor(row = products.indexOf(it), column = 3)) {
                        text(it.popularity)
                    }
                }
            }
